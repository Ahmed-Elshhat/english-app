...
function UpdateEpisodePage() {
  ...
  const [episode, setEpisode] = useState<EpisodeSchema>();
  ...
  useEffect(() => {
    const fetchEpisode = async () => {
      try {
        ...
        console.log("Episode fetched successfully:", data);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (!isFormChanged()) {
      setNoChangeError("No changes detected in the Episode.");
    } else {
  ...
  const validate = () => {
    ...
    // seasonNumber
    if (!form.seasonNumber || isNaN(form.seasonNumber)) {
    ...
    // episodeNumber
    if (!form.episodeNumber || isNaN(form.episodeNumber)) {
  ...
  const isFormChanged = () => {
    ...
    if (
      ...
      form.playlistId !== episode.playlistId ||
      form.seasonNumber !== episode.seasonNumber ||
      form.episodeNumber !== episode.episodeNumber
    ) {
      return true;
    }
  ...
  return (
    <>
      {loading && <Loading />}
      <div className="update-episode">
        <div className="form-container">
          <h2>
            Update Episode <MdLiveTv />
          </h2>
